...
import { useBenchmarkerDialog } from './BenchmarkerDialog.logic';
...
const BenchmarkerDialog = () => {
  const logic = useBenchmarkerDialog();

  const UI = useMemo(
    () => ({
      title:
        logic.type === IbenchmarkerDialogType.ADD
          ? 'Add benchmarker'
          : 'Edit benchmarker',
      buttonLabel: logic.type === IbenchmarkerDialogType.ADD ? 'Add' : 'Edit',
      buttonIcon:
        logic.type === IbenchmarkerDialogType.ADD ? (
          <PlusIcon />
        ) : (
  ...
  return (
    <Dialog.Root open={logic.isOpen === IModals.BENCHMARKER}>
      <Dialog.Content size="4">
        ...
        <Dialog.Description size="2" color="gray" mb="5">
          Handle every information relative to your benchmarker
        </Dialog.Description>
            ...
            <Box>
              <Controller
                ...
                rules={{
                  ...
                  validate: logic.benchmarkerAlreadyExist,
                }}
                render={({ field, fieldState }) => (
                  <>
                    ...
                    <TextField.Root
                      ...
                      value={field.value}
                      disabled={logic.type === IbenchmarkerDialogType.EDIT}
                    />
                    {fieldState.error && (
          ...
          <>
            ...
            <Flex direction="column">
              <Text size="4" weight="bold">
                Import benchmarkers from Excel
              </Text>
              <ImportExcelTemplate />
              <ImportExcel title="Import benchmarkers from Excel" />
            </Flex>
          </>
        )}
      </Dialog.Content>
...
export default BenchmarkerDialog;
